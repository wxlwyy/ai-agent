...
/*
.augmentUserMessage(newUserText)：核心逻辑：这是 Prompt 类提供的一个工具方法。
它不会修改原始的 Prompt 对象（因为 Prompt 是不可变的），而是创建一个新的 Prompt 实例。
在这个新实例中，它将原本的 UserMessage 替换为你刚刚拼接好的 newUserText。*/
